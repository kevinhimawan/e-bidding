<template>
  <div id="Modal-Bidding-Detail" class="modal" tabindex="-1" role="dialog">
          ...
          <form v-if="Object.keys(biddingData).length > 0">
            <div class="form-row">
              <div class="form-group col-lg-4">
                ...
                <input v-on:keyup="checkbidding" class="form-control" v-model="form.bidding" type="number">
    ...
    submit () {
      ...
      if (!this.errordate) {
        if (this.form.status === 'active' && this.form.bidding >= this.biddingData.lowestAsk && this.biddingData.lowestAsk !== null) {
          ...
          this.updateBiddingDetail({data: this.biddingData, changes: this.form, form: 'active'})
        }
      }
    }
  },
