<template>
  <div id="Modal-Selling-Detail" class="modal" tabindex="-1" role="dialog">
          ...
          <form v-if="Object.keys(sellingData).length > 0">
            <div class="form-row">
              <div class="form-group col-lg-4">
                ...
                <input @keyup="checkprice" class="form-control" v-model="form.price" type="number">
                <small v-if="transactionproceed" class="form-text information-text">Transaction will be proceed.</small>
              </div>
              <div class="form-group col-lg-4">
...
export default {
  ...
  methods: {
    ...
    checkprice () {
      this.form.status === 'active' && this.form.price <= this.sellingData.highestBid ? this.transactionproceed = true : this.transactionproceed = false
    },
    changeStatus () {
    ...
    submit () {
      ...
      if (!this.errordate) {
        if (this.form.status === 'active' && this.form.price <= this.sellingData.highestBid && this.sellingData.highestBid !== null) {
          ...
          this.updateSellingDetail({data: this.sellingData, changes: this.form, form: 'active'})
        }
      }
    }
  },
